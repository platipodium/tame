...
varn={'NO3';'NH4';'PO4';'rate';'det_C';'det_N';'det_P';'dom_C';'dom_N';'dom_P';'temp';};
  % convention: env variables (e.g. temp) at the end
...
dxp = 0.96/(ncol+0.05); dyp = 0.96/(nrow +0.05);
x00 = 0.08; y00=0.08;

% open figure
gcf=figure(1);
set(gcf,'Position',[400 00 150+ncol*320 100+nrow*280],'Visible','on','Color','w');clf;
...
for i=1:totn
  ...
  gca=subplot('Position',[x00+ix*dxp y00+iy*dyp 0.86*dxp 0.87*dyp]);
  hold on;
  ...
  j=find(strcmp(vars,[mname varn{i}]));
  ymin=0;ymax=0;
  ...
  % add title
  tmp=strrep(varn{i},'_',' ');
  annotation('textbox',[x00+(ix-0.2)*dxp y00+(iy+0.6)*dyp 0.25 0.07],'String',tmp,'Color','k','Fontweight','bold','FontSize',fs,'LineStyle','none','HorizontalAlignment','center');
