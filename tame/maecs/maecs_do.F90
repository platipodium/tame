...
#ifdef debugMK
   if (nut%N /= nut%N .OR. nut%N < 0.0_RK) then; write (0, *) 'ERROR: maecs_do#161 nut%N = ', nut%N; stop; end if
   if (phy%C /= phy%C .OR. phy%C < 0.0_RK) then; write (0, *) 'ERROR: maecs_do#162 phy%C = ', phy%C; stop; end if
   if (phy%N /= phy%N .OR. phy%N < 0.0_RK) then; write (0, *) 'ERROR: maecs_do#163 phy%N = ', phy%N; stop; end if
   if (det%C /= det%C .OR. det%C < 0.0_RK) then; write (0, *) 'ERROR: maecs_do#164 det%C = ', det%C; stop; end if
   if (det%N /= det%N .OR. det%N < 0.0_RK) then; write (0, *) 'ERROR: maecs_do#165 det%N = ', det%N; stop; end if
   if (dom%C /= dom%C .OR. dom%C < 0.0_RK) then; write (0, *) 'ERROR: maecs_do#166 dom%C = ', dom%C; stop; end if
   if (dom%N /= dom%N .OR. dom%N < 0.0_RK) then; write (0, *) 'ERROR: maecs_do#167 dom%N = ', dom%N; stop; end if
   if (self%RubiscoOn) then
      if (phy%Rub /= phy%Rub .OR. phy%Rub < 0.0_RK) then; write (0, *) 'ERROR: maecs_do#169 phy%Rub = ', phy%Rub; stop; end if
   end if
   if (self%PhotoacclimOn) then
      if (phy%chl /= phy%chl .OR. phy%chl < 0.0_RK) then; write (0, *) 'ERROR: maecs_do#172 phy%chl = ', phy%chl; stop; end if
   end if
   if (self%PhosphorusOn) then
      if (nut%P /= nut%P .OR. nut%P < 0.0_RK) then; write (0, *) 'ERROR: maecs_do#175 nut%P = ', nut%P; stop; end if
      if (phy%P /= phy%P .OR. phy%P < 0.0_RK) then; write (0, *) 'ERROR: maecs_do#176 phy%P = ', phy%P; stop; end if
      if (det%P /= det%P .OR. det%P < 0.0_RK) then; write (0, *) 'ERROR: maecs_do#177 det%P = ', det%P; stop; end if
      if (dom%P /= dom%P .OR. dom%P < 0.0_RK) then; write (0, *) 'ERROR: maecs_do#178 dom%P = ', dom%P; stop; end if
   end if
   if (self%SiliconOn) then
      if (nut%Si /= nut%Si .OR. nut%Si < 0.0_RK) then; write (0, *) 'ERROR: maecs_do#181 nut%Si = ', nut%Si; stop; end if
      if (phy%Si /= phy%Si .OR. phy%Si < 0.0_RK) then; write (0, *) 'ERROR: maecs_do#182 phy%Si = ', phy%Si; stop; end if
      if (det%Si /= det%Si .OR. det%Si < 0.0_RK) then; write (0, *) 'ERROR: maecs_do#183 det%Si = ', det%Si; stop; end if
   end if
   if (self%GrazingOn) then
      if (zoo%C /= zoo%C .OR. zoo%C < 0.0_RK) then; write (0, *) 'ERROR: maecs_do#186 zoo%C = ', zoo%C; stop; end if
   end if
   if (self%BioOxyOn) then
      if (env%nh3 /= env%nh3) then; write (0, *) 'ERROR: maecs_do#189 env%nh3 = ', env%nh3; stop; end if
      if (env%oxy /= env%oxy) then; write (0, *) 'ERROR: maecs_do#190 env%oxy = ', env%oxy; stop; end if
      if (env%odu /= env%odu) then; write (0, *) 'ERROR: maecs_do#191 env%odu = ', env%odu; stop; end if
   end if
   if (self%VirusOn) then
      if (phy%vir /= phy%vir .OR. phy%vir < 0.0_RK) then; write (0, *) 'ERROR: maecs_do#194 phy%vir = ', phy%vir; stop; end if
   end if
   if (self%NResOn) then
      if (env%RNit /= env%RNit) then; write (0, *) 'ERROR: maecs_do#197 env%RNit = ', env%RNit; stop; end if
   end if
   if (env%temp /= env%temp) then; write (0, *) 'ERROR: maecs_do#199 env%temp = ', env%temp; stop; end if
   if (env%par /= env%par) then; write (0, *) 'ERROR: maecs_do#200 env%par = ', env%par; stop; end if
#endif
...
!  vrepl = -self%vir_mu *  phy%relQ%N ! linear dependence on stoichiometry
         ...
         end if   !phy%C* (1.0_rk+phy%reg%C/self%vir_phyC)
